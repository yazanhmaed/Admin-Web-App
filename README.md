...
### كود الشركة (توليد تلقائي)

كود كل شركة يُولَّد بالكامل تلقائياً بصيغة تسلسلية ثابتة `CMP-0001`،
`CMP-0002`، ... — لا يُدخله المستخدم يدوياً ولا يمكن تعديله بالواجهة. يُعرض
عند فتح نموذج "إضافة شركة" كمعاينة فورية (حقل للعرض فقط)، لكن الكود
النهائي الفعلي يُولَّد ذرّياً (atomically) داخل معاملة Firestore
(`meta/companyCodeCounter`) لحظة الحفظ — وليس بالواجهة فقط — لتفادي أي
تعارض أكواد لو حاول أكثر من إدمن الحفظ بنفس اللحظة تقريباً.

### استيراد بيانات Firebase بالسحب والإفلات

نموذج "إضافة/تعديل شركة" يحوي منطقة سحب وإفلات (أو زر استعراض كبديل) لملف
**JSON** يحوي بيانات اتصال Firebase الخاصة بالشركة، لتعبئة حقول
Android/iOS تلقائياً بدل الإدخال اليدوي. شكل الملف المتوقع:
...
* حقل `ios` اختياري بالكامل — يمكن حذفه أو تركه فارغاً.
* أي حقل إضافي غير معروف بالملف يُتجاهل.
* الحد الأقصى لحجم الملف 2MB؛ الصيغ المدعومة حالياً: `.json` فقط.
* بعد الاستيراد يمكن مراجعة/تعديل القيم بحقول النموذج قبل الحفظ النهائي —
  الاستيراد لا يحفظ تلقائياً.
...
* إضافة شركة بكود تسلسلي يُولَّد تلقائياً (CMP-0001...)، مع إمكانية استيراد
  بيانات Firebase من ملف JSON بالسحب والإفلات.
